...
bysort pptid: egen final_waking_from_sleep = max((_n)*(awake==0))
bysort pptid: gen final_woken = 1 if _n >= final_waking_from_sleep
...
bysort pptid: replace label_one = 8 if _n == final_waking_from_sleep-1
bysort pptid: replace label_one = 9 if _n == final_waking_from_sleep
bysort pptid: replace label_one = 10 if _n == final_waking_from_sleep+1
bysort pptid: replace label_one = 11 if (_n > final_waking_from_sleep+1) & (_n < final_waking_from_sleep+10)
capture gen label_two = .
bysort pptid: replace label_two = 12 if (_n > final_waking_from_sleep+1) & (_n < final_waking_from_sleep+10)
bysort pptid: replace label_one = 12 if _n >= final_waking_from_sleep+10
